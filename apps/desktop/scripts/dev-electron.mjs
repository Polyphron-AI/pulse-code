...
const requiredFiles = [
  ...
  "dist-electron/voice-overlay-preload.cjs",
  "../server/dist/bin.mjs",
];
const watchedDirectories = [
  {
    directory: "dist-electron",
    files: new Set(["main.cjs", "preload.cjs", "voice-overlay-preload.cjs"]),
  },
...
const remoteDebuggingPort = (
  process.env.PULSE_CODE_DESKTOP_REMOTE_DEBUGGING_PORT ??
...
if (devProtocolClient) {
  childEnv.PULSE_CODE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.T3CODE_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.PULSE_CODE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
...
function cleanupStaleDevApps() {
  ...
  for (const marker of ["pulse-code-dev-root", "t3code-dev-root"]) {
    NodeChildProcess.spawnSync("pkill", ["-f", "--", `--${marker}=${desktopDir}`], {
      stdio: "ignore",
    });
  }
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--pulse-code-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
