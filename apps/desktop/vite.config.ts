...
const shouldLaunchElectronAfterPack =
  (repoEnv.PULSE_CODE_DESKTOP_DEV ?? repoEnv.T3CODE_DESKTOP_DEV) === "1";
...
export default defineConfig({
  run: {
    tasks: {
      ...
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env PULSE_CODE_DESKTOP_DEV=1 T3CODE_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["t3#build"],
        cache: false,
      },
      "dev:bundle": {
