...
function getAutoUpdateDisabledReason(args: {
  ...
  if (args.disabledByEnv) {
    return "Automatic updates are disabled by the PULSE_CODE_DISABLE_AUTO_UPDATE setting.";
  }
...
export const make = Effect.gen(function* () {
  ...
  const checkForUpdates = Effect.fn("desktop.updates.checkForUpdates")(function* (
    ...
    if (state.rollbackVersion !== null && reason !== "rollback") return false;
          ...
          if (state.rollbackVersion !== null) {
            ...
            yield* Effect.scoped(
              Effect.gen(function* () {
                ...
                if ((yield* Ref.get(updateStateRef)).rollbackVersion === info.version) {
  ...
  const startRollback = Effect.fn("desktop.updates.startRollback")(function* (version: string) {
    ...
    if (!(yield* tryStartUpdateAction("check"))) return { accepted: false, completed: false };
    return yield* Effect.gen(function* () {
      ...
      yield* checkForUpdates("rollback", "held");
  ...
  return DesktopUpdates.of({
    ...
    check: Effect.fn("desktop.updates.check")(function* (reason: string) {
      ...
      const previousState = yield* Ref.get(updateStateRef);
      const checked = yield* resetRollbackPin.pipe(
        Effect.andThen(checkForUpdates(reason, "held")),
        Effect.onInterrupt(() => setState(previousState)),
        Effect.ensuring(finishUpdateAction("check")),
      );
      return {
        checked,
