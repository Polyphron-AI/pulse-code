...
describe("DesktopUpdates", () => {
  it.effect("keeps a Pulse rollback pinned while the update check owns the action", () => {
    const releaseCheck = Deferred.makeUnsafe<void>();
    const checkStarted = Deferred.makeUnsafe<void>();
    ...
    const harness = makeHarness({
      ...
      downloadUpdate: Deferred.succeed(downloadStarted, undefined).pipe(Effect.asVoid),
    });
        ...
        harness
          ...
          .some(
            (feed) =>
              ...
              feed !== null &&
              "url" in feed &&
              String(feed.url).includes("Polyphron-AI/pulse-code/releases/download/v1.2.2"),
          ),
      );
