...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Pulse Next"),
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            // Any T3 Code directory the host happens to have is reported as
            // present, so the assertions below prove Pulse Next ignores it.
            exists: () => Effect.succeed(input.legacyPathExists !== false),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("never adopts a T3 Code userData directory, even when one exists", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/pulsenext");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Pulse Next (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Pulse Next (Alpha)");
