...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Pulse Code"),
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: () => Effect.die("Pulse identity must not inspect legacy T3 user-data paths"),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("always resolves fresh Pulse userData without probing T3 paths", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;
        const normalizedPath = normalizePath(userDataPath);

        assert.equal(normalizedPath, "/Users/alice/Library/Application Support/pulsecode");
        assert.isFalse(normalizedPath.includes("t3code"));
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Pulse Code (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Pulse Code (Alpha)");
