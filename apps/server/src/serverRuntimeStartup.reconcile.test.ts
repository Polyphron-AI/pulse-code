...
const queryWithThreads = (
  ...
  activeScheduledThreadIds: ReadonlyArray<ThreadId> = [],
) =>
  ({
    getCommandReadModel: () =>
      Effect.succeed({
        ...
        schedules: [
          {
            projectStates: activeScheduledThreadIds.map((threadId) => ({
              threadId,
...
const runReconciliation = (input: {
  ...
  readonly activeScheduledThreadIds?: ReadonlyArray<ThreadId>;
  ...
  ServerRuntimeStartup.reconcileProviderSessions.pipe(
    Effect.provideService(
      ...
      queryWithThreads(input.threads, input.activeScheduledThreadIds),
    ),
    Effect.provideService(
...
it.effect.each(
  ...
  "continues Codex and OMP $recovery sessions with a $persistedTurn directory turn",
  ({ recovery, persistedTurn }) =>
    Effect.gen(function* () {
      ...
      const fallbackProviderInstanceId = ProviderInstanceId.make("omp");
...
for (const scenario of [
  ...
  it.effect(`does not recover an interrupted session with ${scenario}`, () => {
    ...
    return runReconciliation({
      ...
      activeScheduledThreadIds: scenario.includes("scheduled occurrence") ? [thread.id] : [],
      directory: {
        getBinding: () =>
          Effect.succeed(
            Option.some({
