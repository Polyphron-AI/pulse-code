...
it.layer(NodeServices.layer)("desktop app update", (it) => {
  it.effect("cancels preparation when the progress stream fails", () =>
    Effect.gen(function* () {
      const { service, cancellations } = yield* makeHarness({
        reports: (requestId) => [report(requestId, makeState({ status: "downloading" }))],
      });
      const failure = new ServerSelfUpdateError({ reason: "client disconnected" });
      expect(yield* service.run(() => Effect.fail(failure)).pipe(Effect.flip)).toBe(failure);
  ...
  it.effect("is unavailable without desktop mode or the control fd", () =>
    Effect.gen(function* () {
      ...
      expect((yield* noFd.service.run(() => Effect.void).pipe(Effect.flip)).reason).toContain(
        "not started by the Pulse Code desktop app",
      );
  ...
  it.effect("maps up-to-date and failed outcomes to readable errors", () =>
    Effect.gen(function* () {
      ...
      expect((yield* upToDate.service.run(() => Effect.void).pipe(Effect.flip)).reason).toBe(
        "The Pulse Code desktop app on this machine is already up to date on 1.2.3.",
      );
