...
// These PATH fixtures use POSIX executable files and cannot be resolved on a Windows filesystem.
// oxlint-disable-next-line t3code/no-global-process-runtime -- host-specific filesystem fixtures.
const posixEffect = process.platform === "win32" ? it.effect.skip : it.effect;
// oxlint-disable-next-line t3code/no-global-process-runtime -- host-specific filesystem fixtures.
const posixLive = process.platform === "win32" ? it.live.skip : it.live;
...
posixEffect("reveals a file in Finder with open -R on macOS", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
  ...
  async () => {
    ...
    try {
      ...
      const source = ExternalLauncher.buildFileExplorerRevealPowerShellSource(
        recorderPath,
        explorerTarget,
      ).replace("Start-Process ", "Start-Process -Wait -WindowStyle Hidden ");
      ...
      // The test recorder is joined explicitly; no polling is needed.
...
posixEffect("reveals a WSL file in Windows File Explorer through its UNC path", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
...
posixEffect("does not advertise reveal from WSL when interop PowerShell is missing", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
...
posixEffect("reveals through the Linux file manager when WSL lacks interop PowerShell", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
...
posixEffect("falls back to the Linux file manager when WSL lacks the Explorer bridge", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
...
posixEffect("reveals by opening the containing directory on Linux", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
...
posixEffect("advertises a Linux file manager when a directory handler is installed", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
...
posixLive("a stalled handler probe drops only the file manager", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
