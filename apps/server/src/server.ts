...
// Pulse Code's primary transport is long-lived WebSocket RPC, whose Effect scope finalizer
...
const RuntimeCoreDependenciesLive = ReactorLayerLive.pipe(
  ...
  Layer.provideMerge(PersistenceLayerLive),
  Layer.provideMerge(
    Layer.mergeAll(Keybindings.layer, UsageLimitSources.layer, EnvironmentTheme.layer),
  ),
  ...
  // logger instances.
  Layer.provideMerge(Layer.mergeAll(ProviderEventLoggers.layer, CodexResetCredit.layer)),
...
const IssuesServiceLive = IssuesService.layer.pipe(
  Layer.provide(IssuesStoreLive),
  Layer.provide(PulseIssuesClient.layer),
  Layer.provide(ServerSecretStore.layer),
);
        ...
        yield* forkParked(
          Effect.gen(function* () {
            ...
            yield* reconcileDesiredCloudLink(`http://127.0.0.1:${address.port}`).pipe(
              ...
              Effect.tap(() => Effect.logInfo("Pulse Connect desired link reconciled on startup")),
              Effect.catch((cause) =>
                Effect.logWarning("Failed to reconcile Pulse Connect desired link on startup", {
                  cause,
                }),
              ),
            );
