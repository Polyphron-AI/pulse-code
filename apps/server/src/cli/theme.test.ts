...
const runCli = (
  ...
  denied?: { path: string; operation: "read" | "write" },
) =>
  Effect.gen(function* () {
    ...
    const failure = () =>
      Effect.fail(
        PlatformError.systemError({
          ...
          module: "FileSystem",
          method: denied?.operation ?? "read",
          pathOrDescriptor: denied?.path ?? "",
          cause: new Error("synthetic denied operation"),
        }),
      );
    return yield* Command.runWith(cli, { version: "0.0.0" })(args).pipe(
      Effect.provideService(FileSystem.FileSystem, {
        ...fs,
        readFileString: (path, ...rest) =>
          denied?.operation === "read" && path === denied.path
            ? failure()
            : fs.readFileString(path, ...rest),
        rename: (from, to) =>
          denied?.operation === "write" && to === denied.path ? failure() : fs.rename(from, to),
      }),
    );
...
describe("t3 theme", () => {
  ...
  // settings rename is denied after the publish, exercising rollback on every platform.
  it.effect("rolls back a publish when the default cannot be written", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* runCli(["theme", "set", themeFile, "--base-dir", baseDir], {
        path: settingsPathFor(baseDir),
        operation: "write",
      }).pipe(Effect.flip);
  ...
  it.effect("refuses to write when the settings file cannot be read", () =>
    Effect.gen(function* () {
      ...
      const failure = yield* runCli(["theme", "set", "ocean", "--base-dir", baseDir], {
        path: settingsPathFor(baseDir),
        operation: "read",
      }).pipe(Effect.flip);
