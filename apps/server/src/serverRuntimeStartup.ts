...
export const reconcileProviderSessions = Effect.gen(function* () {
  ...
  const { threads, schedules } = yield* query.getCommandReadModel();
  // Scheduled occurrences own one turn. A resumed provider's ready event could
  // settle that old turn before continuation and incorrectly publish a handoff.
  const activeScheduledThreads = new Set(
    (schedules ?? []).flatMap((schedule) =>
      schedule.projectStates.flatMap((state) =>
        state.lastOccurrenceStatus === "running" && state.threadId !== null ? [state.threadId] : [],
      ),
    ),
  );
  ...
  for (const thread of orphanedThreads) {
    ...
    if (
      ...
      !activeScheduledThreads.has(thread.id) &&
          ...
          if (serverConfig.startupPresentation === "headless") {
            ...
            if (serverConfig.mode !== "desktop") {
              yield* Effect.logInfo(
                "Authentication required. Open Pulse Code using the pairing URL.",
