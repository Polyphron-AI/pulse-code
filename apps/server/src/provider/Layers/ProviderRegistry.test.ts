    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
            ...
            Layer.updateService(ChildProcessSpawner.ChildProcessSpawner, (spawner) =>
              ChildProcessSpawner.make((command) => {
                ...
                spawnedCommands.push(
                  /(?:^|[\\/])omp(?:\.exe)?$/i.test(command.command) ? "omp" : command.command,
                );
                const beforeSpawn =
          ...
          yield* Effect.gen(function* () {
            ...
            // Wait for the replacement executable, then observe its distinct catalog result.
            ...
            assert.deepStrictEqual(
              spawnedCommands.filter((command) => command !== "omp"),
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Pulse Code settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Pulse Code settings.");
        }),
      );
    });
