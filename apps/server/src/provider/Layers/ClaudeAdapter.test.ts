...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("puts the command text last so attachments do not suppress expansion", () => {
    ...
    return Effect.gen(function* () {
      ...
      for (const attachment of [imageAttachment]) {
        const attachmentPath = NodePath.join(attachmentsDir, attachmentRelativePath(attachment)!);
        NodeFS.mkdirSync(NodePath.dirname(attachmentPath), { recursive: true });
        NodeFS.writeFileSync(attachmentPath, Uint8Array.from([1, 2, 3, 4]));
      }
  ...
  for (const pendingCount of [1, 65]) {
    it.effect(`subagent model snapshot retention with ${pendingCount} pending entries`, () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        for (let index = 0; index < pendingCount; index++) {
          harness.query.emit({
            type: "assistant",
            parent_tool_use_id: index === 0 ? "toolu_agent_early" : `unmatched-${index}`,
            message: {
              model: "claude-sonnet-5[1m]",
              content: [],
            },
            uuid: `early-snapshot-${index}`,
            session_id: "sdk-session",
          } as unknown as SDKMessage);
        }
        harness.query.emit({
        ...
        if (started?.type === "task.started") {
          assert.equal(
            ...
            pendingCount === 1 ? "claude-sonnet-5[1m]" : SYNTHETIC_CLAUDE_CAPABLE_MODEL,
          );
          assert.equal(started.payload.effort, "max");
        }
        const progress = taskEvents[1];
  ...
  it.effect("drops an unusable Claude reset time, not the row or the session", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Finish the parked turn before starting another one; Pulse waits for
      // the active turn to finish rather than implicitly replacing it.
      harness.query.emit({
        type: "result",
        subtype: "success",
        is_error: false,
  ...
  it.effect("warns for unknown Claude limits with generic model bucket names", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const { runtimeEvents, runtimeEventsFiber, drainSdkMessages } =
      ...
      assert.deepEqual(
        ...
        [
          ...
          "Claude usage limit reached. This turn is paused until the 7-day model limit resets in 1h.",
        ],
      );
      assert.equal(
        runtimeEvents.filter((event) => event.type === "account.rate-limits.updated").length,
  ...
  it.effect("publishes before and after token counts when Claude compacts", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const session = yield* adapter.startSession({
      ...
      yield* Stream.take(adapter.streamEvents, 3).pipe(Stream.runDrain);
      const compacted = yield* adapter.streamEvents.pipe(
        Stream.filter(
      ...
      harness.query.emit({
        ...
        session_id: "sdk-compaction",
        uuid: "compaction-counts",
      } as unknown as SDKMessage);
      const event = yield* Fiber.join(compacted);
      assert.equal(event._tag, "Some");
      if (event._tag === "Some" && event.value.type === "thread.state.changed") {
        assert.equal(event.value.payload.beforeTokens, 200);
        assert.equal(event.value.payload.afterTokens, 40);
      }
  ...
  it.effect("routes Claude resume compaction through the shared user-input UI", () => {
    ...
    return Effect.gen(function* () {
      ...
      const dialogPromise = onUserDialog(
        ...
        { signal: new AbortController().signal, requestId: "resume-dialog" },
      );
