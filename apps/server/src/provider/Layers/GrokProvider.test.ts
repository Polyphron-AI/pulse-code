...
// Fixture executables must match the real test host, not an injected provider platform.
// oxlint-disable-next-line t3code/no-global-process-runtime
const isWindowsTestHost = process.platform === "win32";
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const grokPath = path.join(dir, isWindowsTestHost ? "grok.cmd" : "grok");
          yield* fs.writeFileString(
            grokPath,
            isWindowsTestHost
              ? `@echo off\r\necho ${secretStderr} 1>&2\r\nexit /b 2\r\n`
  ...
  const writeFakeGrokCli = (input: { readonly modelsOutput: string; readonly acp: boolean }) =>
    Effect.gen(function* () {
      ...
      if (isWindowsTestHost) {
        const runnerPath = path.join(dir, "grok.mjs");
        const wrapperPath = path.join(dir, "grok.cmd");
        // These values are JavaScript literals in a synthetic executable fixture.
        // @effect-diagnostics-next-line preferSchemaOverJson:off
        const modelsLiteral = JSON.stringify(input.modelsOutput);
        // @effect-diagnostics-next-line preferSchemaOverJson:off
        const moduleLiteral = JSON.stringify(NodeURL.pathToFileURL(mockAgentPath).href);
        const program = `const command = process.argv[2];
if (command === "--version") { console.log("grok 1.0.13"); }
else if (command === "models") { console.log(${modelsLiteral}); }
else if (command === "agent") { ${input.acp ? `await import(${moduleLiteral});` : "process.exit(3);"} }
else { process.exit(1); }
`;
        yield* fs.writeFileString(runnerPath, program);
        yield* fs.writeFileString(
          wrapperPath,
