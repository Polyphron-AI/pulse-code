...
async function makeMockGrokWrapper(extraEnv?: Record<string, string>) {
  ...
  if (isWindowsTestHost) {
    const runnerPath = NodePath.join(dir, "fake-grok.mjs");
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  ...
  // Windows terminates processes without delivering the POSIX SIGTERM handler.
  it.effect.skipIf(isWindowsTestHost)("closes the ACP child process when a session stops", () =>
    Effect.gen(function* () {
  ...
  it.effect("does not time out a Grok turn before ACP emits progress", () =>
    Effect.gen(function* () {
      ...
      const steerSendTurnFiber = yield* adapter
        .sendTurn({ threadId, input: "keep reasoning", attachments: [], busyBehavior: "steer" })
        .pipe(Effect.forkChild);
  ...
  it.effect("refreshes Grok liveness when a turn is steered", () =>
    Effect.gen(function* () {
      ...
      yield* TestClock.adjust("999 millis");
      const steerSendTurnFiber = yield* adapter
        .sendTurn({ threadId, input: "continue working", attachments: [], busyBehavior: "steer" })
        .pipe(Effect.forkChild);
      for (let yieldAttempt = 0; yieldAttempt < 12; yieldAttempt += 1) {
        yield* Effect.yieldNow;
      }
  ...
  it.effect("cancels an in-flight prompt when a mid-turn sendTurn steers", () =>
    Effect.gen(function* () {
      ...
      const steered = yield* adapter
        .sendTurn({ threadId, input: "take this instead", attachments: [], busyBehavior: "steer" })
