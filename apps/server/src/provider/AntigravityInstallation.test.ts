...
const makeHarness = Effect.fn("test.makeAntigravityInstallation")(function* (
  ...
  const trackedFs = FileSystem.FileSystem.of({
    ...installationFs,
    // These Linux fixtures request 0755 files; Windows cannot persist POSIX execute bits.
    stat: (filePath) =>
      installationFs
        .stat(filePath)
        .pipe(
...
it.layer(NodeServices.layer)("Antigravity installation", (it) => {
  it.effect("verifies both files before activating a streamed download", () =>
    Effect.gen(function* () {
      ...
      const completed = yield* terminalState(installation);
      expect(completed, completed.message ?? undefined).toMatchObject({
        phase: "succeeded",
  ...
  it.effect.each([
    ...
    Effect.gen(function* () {
      ...
      const { installation, stagingReleased } = yield* makeHarness({
        ...
        ...(hostPlatform === "win32"
          ? { platform: "win32", archive: Buffer.from(zipFixtures.windows, "base64") }
          : {}),
      }).pipe(Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, spawner));
      yield* installation.start;
      const validationState = yield* terminalState(installation);
      expect(validationState, validationState.message ?? undefined).toMatchObject({
        phase: testCase.valid ? "succeeded" : "failed",
      });
      yield* Deferred.await(stagingReleased);
  ...
  it.effect("honors explicit paths and reports invalid overrides without falling back", () =>
    Effect.gen(function* () {
      ...
      const archive =
        platform === "win32" ? Buffer.from(zipFixtures.windows, "base64") : completeArchive;
      ...
      const isolated = yield* makeHarness({ platform, archive, baseDir });
