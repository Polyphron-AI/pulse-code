...
function acpSpawnEnvironmentOptions(
  spawn: Pick<AcpSpawnInput, "env" | "environmentMode" | "extendEnv">,
):
  | Record<never, never>
  | {
      readonly env: NodeJS.ProcessEnv;
  ...
  const extendEnv = spawn.environmentMode !== "exact" && (spawn.extendEnv ?? true);
