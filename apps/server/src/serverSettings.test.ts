...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("preserves Pulse OMP defaults and explicit disables during provider restoration", () =>
    Effect.gen(function* () {
      const config = yield* ServerConfig.ServerConfig;
      const fs = yield* FileSystem.FileSystem;
      const settings = yield* ServerSettingsModule.ServerSettingsService;
      ...
      yield* fs.writeFileString(
        ...
        JSON.stringify({
          ...
          providerInstances: {
            omp_default: { driver: "omp", config: {} },
            omp_disabled: { driver: "omp", enabled: true, config: { enabled: false } },
          },
        }),
      );
      const restored = yield* settings.getSettings;
      assert.isTrue(restored.providers.cursor.enabled);
      assert.isFalse(restored.providers.omp.enabled);
      assert.isTrue(
        resolveProviderInstanceEnabled(
          restored.providerInstances[ProviderInstanceId.make("omp_default")]!,
        ),
      );
      assert.isFalse(
        resolveProviderInstanceEnabled(
          restored.providerInstances[ProviderInstanceId.make("omp_disabled")]!,
        ),
      );
      assert.deepEqual(
        restored.providerInstances[ProviderInstanceId.make("omp_disabled")]?.config,
        {},
      );
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );
