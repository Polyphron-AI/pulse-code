  ...
  const persistProviderEnvironmentSecrets = (
    ...
    Effect.gen(function* () {
      ...
      for (const [sourceId, source] of Object.entries(next.usageLimitSources)) {
        ...
        if (source.managementKey === USAGE_LIMIT_SOURCE_KEY_REDACTED) {
          const previousKey =
            current.usageLimitSources[sourceId as keyof typeof current.usageLimitSources]
              ?.managementKey;
          if (previousKey && previousKey !== USAGE_LIMIT_SOURCE_KEY_REDACTED) {
            yield* secretStore
              .set(secretName, textEncoder.encode(previousKey))
              .pipe(
                Effect.mapError(
                  (cause) =>
