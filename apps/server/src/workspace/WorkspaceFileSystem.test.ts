...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceFileSystemLive", (it) => {
  describe("readFile", () => {
    ...
    // oxlint-disable-next-line t3code/no-global-process-runtime -- mkfifo is an actual host executable, unavailable on Windows.
    it.effect.skipIf(process.platform === "win32")("rejects a FIFO without blocking on open", () =>
      Effect.gen(function* () {
        const workspaceFileSystem = yield* WorkspaceFileSystem.WorkspaceFileSystem;
        const path = yield* Path.Path;
        const cwd = yield* makeTempDir;
