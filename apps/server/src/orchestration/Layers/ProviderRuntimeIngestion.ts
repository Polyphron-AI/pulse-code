...
const COMPACTION_ACTIVITY_KINDS = ["context-compaction", "context-window.updated"] as const;
...
const STRICT_PROVIDER_LIFECYCLE_GUARD =
  (process.env.PULSE_CODE_STRICT_PROVIDER_LIFECYCLE_GUARD ??
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (
        ...
        const existing = yield* projectionSnapshotQuery.getUserInputActivity({
          threadId: thread.id,
          requestId: ApprovalRequestId.make(event.requestId),
        });
        if (Option.isSome(existing) && existing.value.kind === "user-input.resolved") return;
      }
      ...
      if (
        ...
        const threadDetail = yield* resolveThreadDetail(thread.id, COMPACTION_ACTIVITY_KINDS);
