...
it.layer(NodeServices.layer)("ScheduleReactor", (it) => {
  ...
  it.effect("catches up one occurrence per missed day after a clock jump", () =>
    Effect.gen(function* () {
      ...
      // All catch-up turns land in the one persistent thread.
  ...
  it.effect("environment scope fans out per project independently", () =>
    Effect.gen(function* () {
      const bed = yield* makeTestBed();
      yield* bed.seedProject(projectA);
      ...
      const starts = ofType(bed.dispatched, "schedule.occurrence.start");
      expect(starts.map((command) => command.occurrenceKey).toSorted()).toEqual([
        keyFor("2026-01-02", projectA),
        keyFor("2026-01-02", projectB),
      ]);
      // One persistent thread per targeted project.
      expect(new Set(starts.map((command) => command.threadId)).size).toBe(2);

      // Project A settles; project B's turn is still running.
      yield* bed.dispatch({
        type: "schedule.occurrence.complete",
      ...
      // A fires its next day. B's day-old run trips the watchdog leash this
      // sweep and does not fire yet — the two projects never block each other.
      const nextStarts = ofType(bed.dispatched, "schedule.occurrence.start").slice(2);
      expect(nextStarts.map((command) => command.occurrenceKey)).toEqual([
        keyFor("2026-01-03", projectA),
      ]);
      const stateB = yield* bed.projectState(projectB);
      expect(stateB).toMatchObject({
        lastOccurrenceKey: keyFor("2026-01-02", projectB),
        lastOccurrenceStatus: "failed",
        lastOccurrenceFailureReason: "timeout:run",
      });

      // The following sweep catches B up independently of A.
      yield* bed.reactor.sweepNow;
      const catchUpStarts = ofType(bed.dispatched, "schedule.occurrence.start").slice(3);
      expect(catchUpStarts.map((command) => command.occurrenceKey)).toEqual([
        keyFor("2026-01-03", projectB),
      ]);
      const stateA = yield* bed.projectState(projectA);
      expect(stateA).toMatchObject({
        lastOccurrenceKey: keyFor("2026-01-03", projectA),
        lastOccurrenceStatus: "running",
      });
    }).pipe(Effect.provide(TestClock.layer())),
  );
