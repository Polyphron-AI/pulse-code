...
describe("CheckpointReactor", () => {
  ...
  effectIt.effect("finalizes checkpoint files while pull request refresh is blocked", () =>
    Effect.gen(function* () {
      ...
      const harness = yield* Effect.promise(() =>
        createHarness({
          ...
          pullRequestRefresh: Deferred.succeed(refreshStarted, undefined).pipe(
      ...
      harness.provider.emit({
        type: "turn.started",
        eventId: EventId.make("evt-refresh-start"),
      ...
      NodeFS.writeFileSync(NodePath.join(harness.cwd, "final.ts"), "final files\n");
      yield* Effect.gen(function* () {
        harness.provider.emit({
          type: "turn.completed",
          eventId: EventId.make("evt-refresh-complete"),
          provider: ProviderDriverKind.make("codex"),
          createdAt,
          threadId,
          turnId,
        ...
        expect(
          gitShowFileAtRef(harness.cwd, checkpointRefForThreadTurn(threadId, 1), "final.ts"),
        ).toBe("final files\n");
