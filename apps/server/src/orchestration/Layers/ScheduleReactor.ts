  ...
  Effect.gen(function* () {
    ...
    const fireOccurrence = (input: {
      ...
      Effect.gen(function* () {
        ...
        if (existingThread === null && modelSelection === null) {
          yield* Effect.logWarning("schedule.reactor.no-model-selection", {
            scheduleId: schedule.id,
            projectId,
            dateLocal,
          });
    ...
    const sweep = Effect.gen(function* () {
      ...
      for (const schedule of model.schedules ?? []) {
        ...
        for (const { projectId, dueDates } of dueFires) {
          for (const dateLocal of dueDates) {
            yield* fireOccurrence({ schedule, projectId, dateLocal, model, nowIso }).pipe(
              Effect.catch((error) =>
