...
it.layer(TestLayer)("ScheduleHandoffGit", (it) => {
  it.scoped("commits only the generated handoff and preserves unrelated staged files", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
  ...
  it.scoped("commits an idempotent date-shaped .gitignore rule", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
  ...
  it.scoped("refuses to absorb existing .gitignore changes", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "schedule-handoff-dirty-ignore-" });
      yield* initializeRepository(root);
