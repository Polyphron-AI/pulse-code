...
export interface OrchestrationEngineShape {
  ...
  /**
   * Subscribe to domain events, returning a stream backed by a subscription
   * that is established synchronously (not lazily on first pull, unlike
   * `streamDomainEvents`). Use this when a caller forks the consuming loop
   * and must not miss an event published between the fork and the forked
   * fiber's first tick.
   */
