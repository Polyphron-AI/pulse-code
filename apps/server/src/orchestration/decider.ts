  ...
  switch (command.type) {
    ...
    case "thread.auto-settle": {
      if (
        ...
        readModel.schedules?.some((schedule) =>
          schedule.projectStates.some(
            (state) =>
              state.threadId === command.threadId && state.lastOccurrenceStatus === "running",
          ),
        )
      ) {
      ...
      // Only an explicit manual settlement may dismiss an async question.
      if (
