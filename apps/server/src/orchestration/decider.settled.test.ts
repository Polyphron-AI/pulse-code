...
it.layer(NodeServices.layer)("settled thread decider", (it) => {
  ...
  it.effect("async questions do not bypass active sessions or native blockers", () =>
    Effect.gen(function* () {
      ...
      for (const blocker of ["running", "starting", "approval", "native", "automatic"] as const) {
        const error = yield* decideOrchestrationCommand({
          command: {
            type: blocker === "automatic" ? "thread.auto-settle" : "thread.settle",
