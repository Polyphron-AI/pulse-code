...
export function assetResponseHeaders(filePath: string, download = false): Record<string, string> {
  return {
    ...
    ...(download
      ? {
          ...
          "Content-Disposition": `attachment; filename*=UTF-8''${encodeURIComponent(filePath.replaceAll("\\", "/").split("/").at(-1) ?? "download").replace(/['()*]/g, (character) => `%${character.charCodeAt(0).toString(16).toUpperCase()}`)}`,
        }
      : {}),
...
export const assetRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    return yield* HttpServerResponse.file(asset.path, {
      status: 200,
      headers: {
        ...assetResponseHeaders(asset.path, asset.download || Boolean(asset.downloadName)),
        ...(asset.downloadName
          ? {
