...
export function assetResponseHeaders(
  ...
  const options =
    typeof input === "boolean"
      ? {
          download: input,
          fileName: filePath.replaceAll("\\", "/").split("/").at(-1) ?? "download",
        }
      : input;
  ...
  return {
    ...
    ...(options?.download
      ? {
          "Content-Disposition":
            input === true
              ? `attachment; filename*=UTF-8''${encodeURIComponent(options.fileName!.toWellFormed()).replace(/['()*]/g, (character) => `%${character.charCodeAt(0).toString(16).toUpperCase()}`)}`
              : downloadContentDisposition(options.fileName),
...
export const assetFileResponse = Effect.fn("assetFileResponse")(function* (
  ...
  const headers = assetResponseHeaders(
    asset.path,
    asset.downloadName
      ? { ...asset, download: true, fileName: asset.downloadName }
      : asset.download && asset.fileName === undefined && asset.mimeType === undefined
        ? true
        : asset,
  );
  if (asset.downloadName) headers["Cache-Control"] = "private, no-store";
...
export const browserApiCorsLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    //
    // PULSE_CODE_DEV_ALLOWED_ORIGINS covers dev servers reached from a second
