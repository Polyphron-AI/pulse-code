...
async function main(): Promise<void> {
  const baseDir = (process.env.PULSE_CODE_HOME ?? process.env.T3CODE_HOME)?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error(
      "PULSE_CODE_HOME (or the supported T3CODE_HOME alias) is required by the Pulse Code service launcher.",
    );
  }
