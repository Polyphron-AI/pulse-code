...
// @effect-diagnostics nodeBuiltinImport:off - node:os resolves the shared Pulse Code home guard.
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Pulse Code once to run migrations.`;
  }
}
...
export const t3SqliteStateCommand = Command.make(
  "t3-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Pulse Code base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Pulse Code SQLite database with automatic backups for writes.",
  ),
);
