...
let currentModeId = ompMode
  ? (process.env.T3_ACP_OMP_INITIAL_MODE ?? "default")
  : antigravityProfile
    ? "default"
    : "ask";
      ...
      if (completeFirstPromptOnCancel && promptCount === 1) {
        yield* agent.client.sessionUpdate({
          ...
          update: {
            ...
            kind: "execute",
            status: "in_progress",
            rawInput: { command: "echo synthetic-command" },
            content: [
              { type: "content", content: { type: "text", text: "Running a synthetic command." } },
            ],
          },
        });
