import { canArrangeActiveThreads } from "../threads/threadOrder";
...
export function useThreadListActions(): {
  ...
  const moveThread = useCallback(
    async (thread: EnvironmentThreadShell, direction: "up" | "down") => {
      ...
      const shells = appAtomRegistry.get(environmentThreadShells.threadShellsAtom);
      if (section === "active" && !canArrangeActiveThreads(shells, configs)) {
        Alert.alert(
          "Could not move thread",
          "Update connected environments to use server-owned settlement before arranging active threads.",
        );
        return false;
      }
