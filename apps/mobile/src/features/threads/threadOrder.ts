...
/** Legacy local auto-settlement has no canonical section shared by both lists.
 * Keep those rows from becoming invisible anchors in a cross-environment move. */
export function canArrangeActiveThreads(
  ...
  return !threads.some((thread) => {
    ...
    const capabilities = configs.get(thread.environmentId)?.environment.capabilities;
    return capabilities?.threadSettlement === true && capabilities.threadAutoSettlement !== true;
  });
}
