import { canArrangeActiveThreads } from "./threadOrder";
...
describe("arranged pinned membership", () => {
  it("retains Pulse's settled pins while excluding snoozed pins", () => {
    const pin = makeThread({
      id: ThreadId.make("pinned-settled"),
      title: "Pinned settled",
      pinnedAt: NOW,
      settledOverride: "settled",
    });
    const sleeping = makeThread({
      ...pin,
      id: ThreadId.make("pinned-sleeping"),
      snoozedUntil: "2099-01-01T00:00:00.000Z",
    });
    expect(
      getThreadListV2OrderedSection({ threads: [pin, sleeping], section: "pinned", now: NOW }).map(
        (thread) => thread.id,
      ),
    ).toEqual([pin.id]);
  });
...
describe("legacy active arrangement guard", () => {
  it("blocks invisible legacy auto-settled anchors until their environment upgrades", () => {
    const legacy = EnvironmentId.make("legacy");
    const rows = [
      makeThread({ id: ThreadId.make("visible-a"), title: "A", activeOrderKey: "aa" }),
      makeThread({
      ...
      makeThread({ id: ThreadId.make("visible-c"), title: "C", activeOrderKey: "cc" }),
    ];
    const configs = new Map([
      [
        environmentId,
        { environment: { capabilities: { threadSettlement: true, threadAutoSettlement: true } } },
      ],
      [
        legacy,
        { environment: { capabilities: { threadSettlement: true, threadAutoSettlement: false } } },
      ],
    ]);
    expect(canArrangeActiveThreads(rows, configs)).toBe(false);
    configs.set(legacy, {
      environment: { capabilities: { threadSettlement: true, threadAutoSettlement: true } },
    });
    expect(canArrangeActiveThreads(rows, configs)).toBe(true);
  });
});
