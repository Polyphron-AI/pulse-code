...
function SetupButton(props: {
  ...
  return (
    <Pressable
      ...
      className={cn(
        "min-h-11 items-center justify-center border border-border px-4 py-3 active:opacity-70 disabled:opacity-40",
    ...
    >
      <Text
        className={cn(
          ...
          props.destructive && "text-danger-foreground",
        )}
      >
        {props.label}
      </Text>
...
function ProviderSetupScreen({ environmentId, instanceId }: ProviderSetupRouteParams) {
  ...
  const backgroundColor = useThemeColor("--color-sheet");
    ...
    <View className="flex-1 bg-sheet">
      ...
      >
        <View className="flex-row items-center gap-3">
          ...
          <Text className="min-w-0 flex-1 text-base text-foreground">{environmentLabel}</Text>
        </View>
        {!isConnected ? (
          ...
          <View className="gap-3">
            <Text className="text-base text-foreground">
              Could not check your environment access.
            </Text>
            <SetupButton label="Retry" onPress={access.refresh} />
          </View>
        ) : !access.data ? (
          <Text className="text-base text-foreground">Checking environment access.</Text>
        ) : !canOperate ? (
          <Text className="text-base text-foreground">
        ...
        {!config ? (
          ...
          <>
            ...
            {provider.setup?.canInstall ? (
              <View className="gap-3 border-t border-border pt-4">
                ...
                {installation?.message ? (
                  <Text className="text-sm text-foreground">{installation.message}</Text>
                ) : null}
            ...
            {provider.setup?.canAuthenticate ? (
              <View className="gap-3 border-t border-border pt-4">
                    ...
                    {auth.authorizationUrl && auth.phase === "waiting" ? (
                      <>
                        ...
                        <TextInput
                          ...
                          autoComplete="off"
                          autoCorrect={false}
                          className="min-h-12 rounded-none border border-border bg-sheet px-3 py-3 text-base text-foreground"
                ...
                {!provider.enabled && !signedIn ? (
                  <Text className="text-sm text-foreground">Enable Antigravity to sign in.</Text>
                ) : !provider.installed && !signedIn ? (
                  <Text className="text-sm text-foreground">Install Antigravity to sign in.</Text>
                ) : null}
                {authMessage ? (
                  <Text className="text-sm text-foreground">{authMessage}</Text>
                ) : null}
                {authQuery.error ? (
                  <Text className="text-sm text-foreground">
            ...
            {provider.message ? (
              <Text className="text-sm text-foreground">{provider.message}</Text>
            ) : null}
          </>
        )}
        {error ? (
          <Text accessibilityRole="alert" className="text-base text-danger-foreground">
            {error}
          </Text>
        ) : null}
        {busy ? (
