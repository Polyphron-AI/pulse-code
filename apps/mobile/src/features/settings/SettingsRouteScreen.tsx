...
function ConfiguredSettingsRouteScreen() {
    ...
    if (result.value.type === "granted") {
      ...
      if (getAgentAwarenessRegistrationStatus() === "registered") {
        ...
        Alert.alert(
          ...
          "Notification access was granted, but this device could not be registered with Pulse Connect. Notifications will start once registration succeeds.",
        );
      }
      return;
    }
  ...
  const promptSignIn = useCallback(() => {
    Alert.alert(
      ...
      "Live Activity updates require Pulse Connect so relay can deliver updates to this device.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Continue",
  ...
  const linkEnvironments = useCallback(async () => {
    ...
    if (getAgentAwarenessRegistrationStatus() === "registered") {
      ...
      Alert.alert(
        ...
        "This device could not be registered with Pulse Connect, so Live Activities won't appear yet. They'll start once registration succeeds.",
      );
    }
  }, [
    connections,
  ...
  const handleDeviceNotificationsChange = useCallback(
    (enabled: boolean) => {
      ...
      Alert.alert(
        ...
        "Notification permission is controlled by iOS. Open Settings to disable notifications for Pulse Code.",
        [
          { text: "Cancel", style: "cancel" },
          { text: "Open Settings", onPress: () => void Linking.openSettings() },
        ],
        ...
        <View className="gap-3">
          <SettingsSection title="Account">
            <SettingsRow
              ...
              label="Pulse Code Account"
              value={accountLabel}
              onPress={openAccount}
            />
          </SettingsSection>
          <Text className="px-2 text-sm text-foreground-muted">
            Pulse Code works locally without signing in. Cloud features are optional.
          </Text>
        </View>
...
function GeneralSettingsSection() {
  ...
  return (
    <SettingsSection title="General">
      ...
      />
      <SettingsRow icon="alarm" label="Scheduled Chats" target="SettingsScheduledChats" />
