...
export function pickComposerImages(input: { readonly existingCount: number }): Promise<{
  readonly images: ReadonlyArray<DraftComposerImageAttachment>;
  readonly error: string | null;
}>;
export function pickComposerImages(input: {
  readonly existingCount: number;
...
export async function pickComposerImages(input: {
  ...
  const nextImages: DraftComposerAttachment[] = [];
  let error: string | null = null;
  ...
  for (const asset of result.assets) {
    ...
    if (asset.type === "video" || mimeType?.startsWith("video/")) {
      ...
      const maxBytes = clampFileAttachmentUploadBytes(input.maxVideoBytes);
      try {
        const { File } = await import("expo-file-system");
        const fileUri = await persistComposerAttachmentFile(asset.uri, name, maxBytes);
        const sizeBytes = new File(fileUri).size ?? asset.fileSize ?? 0;
        if (sizeBytes <= 0 || sizeBytes > maxBytes) {
          await removePersistedComposerAttachmentFile(fileUri);
          throw new Error(
            sizeBytes <= 0
              ? `'${name}' is empty or could not be read.`
              : fileAttachmentTooLargeMessage(name, maxBytes),
          );
        }
        nextImages.push({
          id: uuidv4(),
