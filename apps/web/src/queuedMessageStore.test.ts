...
describe("queuedMessageStore", () => {
  ...
  it("keeps pinned skill revisions through take and retry", () => {
    const { enqueue, take, holdAtFront } = useQueuedMessageStore.getState();
    const entry = enqueue("thread-a", {
      ...makeMessage("first"),
      pulseSkills: [{ id: "review", revision: "abc123" }],
    });

    const taken = take("thread-a", entry.id, null)!;
    holdAtFront("thread-a", taken);
