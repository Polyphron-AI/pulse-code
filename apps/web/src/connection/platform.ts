...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Pulse Code Desktop" : "Pulse Code Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Pulse Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Pulse Connect session is unavailable.",
          });
        }
        return token;
      }),
