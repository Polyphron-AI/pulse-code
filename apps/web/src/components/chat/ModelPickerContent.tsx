...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  const {
    ...
    instanceEntries,
    getModelDisabledReason: suppliedModelDisabledReason,
  ...
  const getModelDisabledReason = useCallback(
    (instanceId: ProviderInstanceId, model: string) => {
      if (
        modelOptionsByInstance
          .get(instanceId)
          ?.some((option) => option.slug === model && option.isUnavailable)
      ) {
        return "This model is unavailable. Refresh models or choose another model.";
      }
      return suppliedModelDisabledReason?.(instanceId, model) ?? null;
    },
    [modelOptionsByInstance, suppliedModelDisabledReason],
  );
