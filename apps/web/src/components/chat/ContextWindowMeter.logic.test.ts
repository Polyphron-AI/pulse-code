...
describe("providerSupportsManualCompaction", () => {
  it("requires a server-advertised command, including on older environments", () => {
    const [provider] = deriveProviderInstanceEntries([
      claudeProvider({ instanceId: "claude", continuationGroupKey: "claude:default" }),
    ]);
    expect(providerSupportsManualCompaction(provider)).toBe(true);
    expect(
      providerSupportsManualCompaction(
        provider && { ...provider, snapshot: { ...provider.snapshot, slashCommands: [] } },
      ),
    ).toBe(false);
    expect(providerSupportsManualCompaction(undefined)).toBe(false);
  });
});
