...
import {
  ...
  composerDraftHasEnvironmentContent,
...
function ChatViewContent(props: ChatViewProps) {
  ...
  }, [threadActivities, latestCheckpointCompletedAt]);
  ...
  useWorkspaceMutationRefresh({
    ...
    resourceKey: JSON.stringify([activeThreadKey, gitStatusCwd]),
  });
  ...
  const composerHasBoundContent = useComposerDraftStore((store) =>
    composerDraftHasEnvironmentContent(store.getComposerDraft(composerDraftTarget)),
  );
  const automaticEnvironment = Boolean(
    ...
    (!composerHasBoundContent || Boolean(draftThread?.loadBalancedEnvironmentId)) &&
  ...
  useEffect(() => {
    ...
    if (
      !current ||
      current.environmentSelection === "manual" ||
      current.loadBalancedEnvironmentId ||
      current.worktreePath ||
      (current.branch && current.environmentSelection !== "auto") ||
      composerDraftHasEnvironmentContent(store.getComposerDraft(draftId))
    )
      return;
  ...
  const onAutoEnvironment = useCallback(() => {
    ...
    if (composerHasBoundContent) {
      toastManager.add({
        type: "warning",
        id: "load-balancing-attachments",
        title: "Keep project context on this machine",
        description: "Remove attachments and project context before choosing automatic routing.",
      });
      return;
    }
  ...
  const linkedThreadPullRequest =
    (isServerThread ? activeThreadShell?.linkedPullRequest : null) ??
  ...
  // the sent message can stay near the top. Pulse Code only owns streaming adjustments
      ...
      activeThreadId === null || !isServerThread
        ...
        : resolveThreadReferenceCopyTarget({
            ...
            linkedPullRequestUrl:
              (linkedThreadPullRequest ?? activeThread?.branchPullRequest)?.url ?? null,
          }),
    [
      activeThreadId,
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Pulse Code server to browse pull requests."
      />
      ...
      <IssueDetailPanel
        key={`${renderedRightPanelSurface.environmentId}:${renderedRightPanelSurface.issueId}`}
        environmentId={renderedRightPanelSurface.environmentId as EnvironmentId}
        reference={{
          projectId: renderedRightPanelSurface.projectId as ProjectId,
          issueId: renderedRightPanelSurface.issueId as IssueId,
        }}
