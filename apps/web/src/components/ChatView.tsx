...
export default function ChatView(props: ChatViewProps) {
  ...
  const onSend = async (
    ...
    const pulseSkills = (queuedMessage?.pulseSkills ?? sendCtx.pulseSkills).map(
    ...
    const preparedDraftSnapshot = queuedMessage
      ? undefined
      : useComposerDraftStore.getState().getComposerDraft(composerDraftTarget);
    const mcpPreparation = await prepareMcpSubmission({
      ...
      isCurrent: () =>
        mcpSubmissionIdentityRef.current === routeThreadKey &&
        (queuedMessage !== undefined || promptRef.current === promptForSend),
    });
