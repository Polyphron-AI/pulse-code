...
> & {
  ...
  const authQuery = useEnvironmentQuery(
    provider.setup?.canAuthenticate ? serverEnvironment.providerAuthState(target) : null,
  );
  const installQuery = useEnvironmentQuery(
    provider.setup?.canInstall ? serverEnvironment.providerInstallState(target) : null,
  );
