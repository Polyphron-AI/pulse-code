...
describe("SidebarUpdateReleaseNotes", () => {
  ...
  it("shows a normal release link for persisted notes without item counts", () => {
    const state = {
      ...baseState,
      releaseNotes: [{ version: "0.0.36-nightly.3", items: ["Older cached change"] }],
    } as unknown as DesktopUpdateState;
    const anchors = collectAnchors(renderNotes(state));
    expect(anchors).toHaveLength(1);
    expect(textContent(anchors[0])).toContain("View release on GitHub");
    expect(textContent(anchors[0])).not.toContain("NaN");
  });
  ...
  it("links each preview to its exact release and labels hidden changes", () => {
    ...
    expect(anchors.map(({ props }) => props.href)).toEqual([
      "https://github.com/Polyphron-AI/pulse-code/releases/tag/v0.0.36-nightly.3",
      "https://github.com/Polyphron-AI/pulse-code/releases/tag/v0.0.36-nightly.2",
      "https://github.com/Polyphron-AI/pulse-code/releases/tag/v0.0.36-nightly.1",
    ]);
  ...
  it("links omitted releases to release history", () => {
    ...
    expect(anchors.at(-1)?.props.href).toBe("https://github.com/Polyphron-AI/pulse-code/releases");
