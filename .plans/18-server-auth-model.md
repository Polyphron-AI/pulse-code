...
Define the long-term server auth architecture for Pulse Code before first-class remote environments ship.
...
Every privileged surface of the Pulse Code server must go through the same auth policy engine:
...
DesktopMain -> Tunnel : publish local Pulse Code endpoint
...
Participants:
  ...
  RemoteT3      = remote Pulse Code server
  Frontend      = desktop renderer
