...
export const HermesSettings = makeProviderSettingsSchema(
  {
    ...
    contextCompressionThreshold: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "Context compression threshold (%)",
        description:
          "How full the context can get before Hermes compresses it (1-100). Written to the Hermes CLI config; leave empty to keep Hermes' own setting.",
        providerSettingsForm: { placeholder: "50", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
  ...
  {
    order: ["binaryPath", "contextCompressionThreshold"],
  },
);
...
const HermesSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  contextCompressionThreshold: Schema.optionalKey(TrimmedString),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
});
