...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Desktop-only. Legacy booleans decode to their equivalent mode and encode
  // back as the canonical string value.
  ...
  // Desktop scroll collapse never follows blur.
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Pulse Code spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
  ...
  sidebarAutoSettleOnMerge: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  /** Explicit environment opt-in; independent of agent browser access. */
  enableAgentWardenAccess: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
  ...
  // Keyed by a user-chosen id so a source keeps its rows across edits.
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  enableAgentWardenAccess: Schema.optionalKey(Schema.Boolean),
