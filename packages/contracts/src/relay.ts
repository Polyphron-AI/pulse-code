...
}).annotate({ description: "Links an authenticated cloud user to a Pulse Code environment." });
...
const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Pulse Connect user.",
  ),
);
...
/**
 * Pulse Code uses new client ids for newly minted relay credentials, while the
 * legacy ids remain accepted for already-installed Pulse Code clients.
 */
export const RelayMobileClientId = "pulse-mobile" as const;
export const RelayWebClientId = "pulse-web" as const;
export const LegacyRelayMobileClientId = "t3-mobile" as const;
export const LegacyRelayWebClientId = "t3-web" as const;
export const RelayMobileClientIds = [RelayMobileClientId, LegacyRelayMobileClientId] as const;
export const RelayWebClientIds = [RelayWebClientId, LegacyRelayWebClientId] as const;
export const RelayPublicClientId = Schema.Literals([...RelayMobileClientIds, ...RelayWebClientIds]);
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Pulse Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Pulse Code environments, connecting authorized clients, and publishing agent activity.",
  );
