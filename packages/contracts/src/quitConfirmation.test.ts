...
describe("quit confirmation compatibility", () => {
  ...
  it.each(["direct", "hold", "double-click"] as const)("round-trips %s", (mode) => {
    ...
    expect(Schema.decodeUnknownSync(ClientSettingsPatch)({ confirmQuit: mode }).confirmQuit).toBe(
      mode,
    );
  });
  it.each([
  ...
  it("requires a canonical mode for new patches", () => {
    expect(() => Schema.decodeUnknownSync(ClientSettingsPatch)({ confirmQuit: true })).toThrow();
  });
});
