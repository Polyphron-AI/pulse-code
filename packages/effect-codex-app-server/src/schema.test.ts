...
it.each(["rateLimitExceeded", "misalignmentPolicyViolation"])(
  "preserves Codex %s errors in history and turn completion",
  (codexErrorInfo) => {
    const failedThread = {
      cliVersion: "0.150.0",
      createdAt: 0,
    ...
    assert.equal(decodedResume.thread.turns[0]?.error?.codexErrorInfo, codexErrorInfo);
    assert.equal(
