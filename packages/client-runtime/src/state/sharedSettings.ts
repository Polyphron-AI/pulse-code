 ...
 * them (restart recovery runs with no client attached). A user does not want
...
/** Build writes only for connected, supported targets; reconnect never writes automatically. */
export function sharedServerSettingsWrites(
  patch: ServerSettingsPatch,
  environments: ReadonlyArray<SharedSettingsEnvironment>,
) {
  const { sharedPatch } = splitSharedServerPatch(patch);
  return environments.flatMap((environment) => {
    if (!environment.syncEligible || environment.settings === null) return [];
    const targetPatch = filterSharedServerPatch(sharedPatch, environment.capabilities);
    return Object.keys(targetPatch).length === 0
      ? []
      : [{ environmentId: environment.environmentId, input: { patch: targetPatch } }];
  });
}
