...
describe("snapshot replay", () => {
  it("keeps an async dismissal final around a full retained activity window", () => {
    ...
    const resolved = makeActivity({
      kind: "user-input.resolved",
      payload: { requestId: "retained-question" },
    });
    const history = Array.from({ length: 500 }, () => makeActivity({}));
    expect(derivePendingRequests([request, ...history]).userInputs).toHaveLength(1);
    expect(derivePendingRequests([resolved, ...history, request]).userInputs).toEqual([]);
    expect(derivePendingRequests([request, ...history, resolved]).userInputs).toEqual([]);
  });
});
