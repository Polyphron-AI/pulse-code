...
describe("persistent project favicon cache", () => {
  ...
  it("finishes earlier writes before removing an environment so restart cannot restore it", async () => {
    const { storage, load, records } = fixture();
    const writeStarted = deferred<void>();
    const releaseWrite = deferred<void>();
    const delayedStorage: ProjectFaviconStorage = {
      ...storage,
      put: async (key, entry) => {
    ...
    const cache = createProjectFaviconCache({ storage: delayedStorage, load });
    await cache.resolve(target, url, signal());
