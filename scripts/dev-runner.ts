...
import { loadRepoEnv, projectPulseCodeEnvAliases } from "./lib/public-config.ts";
...
/**
 * `role` matters because only the backend honours `--host`/`PULSE_CODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("PULSE_CODE_PORT_OFFSET"),
...
const pulseCodeConfig = <A>(legacyName: string, makeConfig: (name: string) => Config.Config<A>) =>
  withLegacyConfigAlias(
    makeConfig(legacyName.replace(/^T3CODE_/, "PULSE_CODE_")),
    makeConfig(legacyName),
  );

const optionalStringConfig = (legacyName: string): Config.Config<string | undefined> =>
  pulseCodeConfig(legacyName, Config.string).pipe(
    Config.option,
    Config.map((value) => Option.getOrUndefined(value)),
  );
const optionalBooleanConfig = (legacyName: string): Config.Config<boolean | undefined> =>
  pulseCodeConfig(legacyName, Config.boolean).pipe(
    Config.option,
    Config.map((value) => Option.getOrUndefined(value)),
  );
const optionalPortConfig = (legacyName: string): Config.Config<number | undefined> =>
  pulseCodeConfig(legacyName, Config.port).pipe(
    Config.option,
    Config.map((value) => Option.getOrUndefined(value)),
  );
const optionalIntegerConfig = (legacyName: string): Config.Config<number | undefined> =>
  pulseCodeConfig(legacyName, Config.int).pipe(
    Config.option,
    Config.map((value) => Option.getOrUndefined(value)),
  );
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "PULSE_CODE_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `PULSE_CODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric PULSE_CODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed PULSE_CODE_DEV_INSTANCE=${seed}` });
  }
...
function setPulseCodeEnv(
  ...
  if (value === undefined) {
    delete environment[canonicalName];
    delete environment[legacyName];
    return;
  }
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3 > ambient PULSE_CODE_HOME) is resolved
    ...
    const output: NodeJS.ProcessEnv = projectPulseCodeEnvAliases({
      ...baseEnv,
      PORT: String(webPort),
      VITE_DEV_SERVER_URL:
        devUrl?.toString() ??
    ...
    if (configuredBaseDir !== undefined) {
      setPulseCodeEnv(output, "HOME", resolvedBaseDir);
    } else {
      setPulseCodeEnv(output, "HOME", undefined);
    }
    ...
    // agent working inside Pulse Code), these leak through and the child server
    ...
    if (!isDesktopMode) {
      setPulseCodeEnv(output, "PORT", String(serverPort));
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        setPulseCodeEnv(output, "SINGLE_ORIGIN_DEV", "1");
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        setPulseCodeEnv(output, "SINGLE_ORIGIN_DEV", undefined);
      }
    } else {
      setPulseCodeEnv(output, "PORT", String(serverPort));
      ...
      setPulseCodeEnv(output, "SINGLE_ORIGIN_DEV", undefined);
      setPulseCodeEnv(output, "MODE", undefined);
      setPulseCodeEnv(output, "NO_BROWSER", undefined);
      setPulseCodeEnv(output, "HOST", undefined);
    }

    if (!isDesktopMode && host !== undefined) {
      setPulseCodeEnv(output, "HOST", host);
    }

    if (!isDesktopMode) {
      setPulseCodeEnv(output, "NO_BROWSER", browser === true ? "0" : "1");
    }
    ...
    if (autoBootstrapProjectFromCwd !== undefined) {
      ...
      setPulseCodeEnv(output, "AUTO_BOOTSTRAP_PROJECT_FROM_CWD", undefined);
    }

    if (logWebSocketEvents !== undefined) {
      setPulseCodeEnv(output, "LOG_WS_EVENTS", logWebSocketEvents ? "1" : "0");
    } else {
      setPulseCodeEnv(output, "LOG_WS_EVENTS", undefined);
    }

    if (mode === "dev") {
      setPulseCodeEnv(output, "MODE", "web");
      setPulseCodeEnv(output, "DESKTOP_WS_URL", undefined);
    }
 ...
 * target reach it there. When `--host`/`PULSE_CODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["PULSE_CODE_PORT_OFFSET", "PULSE_CODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // outrank an ambient PULSE_CODE_HOME. `--home-dir` still wins.
    ...
    const resolvedT3Home =
      ...
      worktreeHome ??
      (hostEnvironment.PULSE_CODE_HOME?.trim() || undefined) ??
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.PULSE_CODE_HOME ?? env.T3CODE_HOME ?? (yield* DEFAULT_T3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.PULSE_CODE_PORT ?? env.T3CODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
      ...
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          setPulseCodeEnv(
            ...
            "DEV_ALLOWED_ORIGINS",
            [
              env.PULSE_CODE_DEV_ALLOWED_ORIGINS ?? env.T3CODE_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so PULSE_CODE_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.PULSE_CODE_BUNDLED_DEV === undefined && env.T3CODE_BUNDLED_DEV === undefined) {
            setPulseCodeEnv(env, "BUNDLED_DEV", "1");
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Pulse Code data directory; runtime state is stored under userdata (equivalent to PULSE_CODE_HOME; T3CODE_HOME remains supported). Inside a git worktree this defaults to that worktree's own .t3 so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to PULSE_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription(
      "WebSocket event logging toggle (equivalent to PULSE_CODE_LOG_WS_EVENTS).",
    ),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("T3CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to PULSE_CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("T3CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to PULSE_CODE_PORT)."),
