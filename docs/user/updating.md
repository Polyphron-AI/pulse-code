# Keeping Pulse Code in Sync

The Pulse Code web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Pulse Code shows a warning with the right update option for that server.
...
If the server instead runs as the Pulse Code background service, update the service on the host and
...
See [Running Pulse Code in the Background](./background-service.md) for install, status, and removal
commands.
