...
Pulse Code shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Pulse Code directly from a
...
- **Pulse Connect** connects computers that are signed in to your account. Run
  `npx t3 connect` on each computer you want to add, then start Pulse Code or run
...
Saved computers and computers discovered through Pulse Connect are selected by
...
If Pulse Code cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Pulse Code cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Pulse Code checks each selected computer for Claude Code and Codex. If an agent is
...
Pulse Code finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Pulse Code keeps the
...
30 days. You can continue those conversations in Pulse Code.

Conversation import is best effort. Pulse Code keeps the first user prompt and the
...
conversation so Pulse Code does not remove user text. It reads one conversation at
a time, streams past oversized tool payloads, and limits retained history to 32 MiB.
