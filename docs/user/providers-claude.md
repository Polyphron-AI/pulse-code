...
This guide is for people who want to use more than one Claude setup in Pulse Code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Pulse Code](./install.md).
...
In Pulse Code Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Pulse Code uses Claude Code's normal config directory.

When you set this field, Pulse Code points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Pulse Code looks.

Then add another Claude provider in Pulse Code:
...
Pulse Code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Pulse Code keeps separate config directories isolated
...
Add or edit a Claude provider in Pulse Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Pulse Code stores the value as a server secret and does not
...
Pulse Code does not need a special Claude Code Router provider. Treat the router as a Claude
