...
This guide is for people who want to use more than one Codex account in Pulse Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Pulse Code](./install.md).
...
- both accounts can see the same Pulse Code/Codex sessions
...
In Pulse Code Settings, name it something obvious:
...
In Pulse Code Settings, add another Codex provider:
...
Pulse Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Pulse Code treats it
