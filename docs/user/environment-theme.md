...
machine running your Pulse Code server does that, its themes appear in the theme library alongside
your own, and Pulse Code follows them: change the desktop theme and Pulse Code retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. Pulse Code Mobile keeps its own
...
the standard Pulse Code look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Pulse Code state
...
exported from Pulse Code (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Pulse Code generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Pulse Code theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Pulse Code does not know is ignored rather
