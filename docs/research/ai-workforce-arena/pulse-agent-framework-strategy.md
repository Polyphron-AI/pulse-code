...
Orca is a separate application. It cannot read Pulse's current `ServerSecretStore` or a future Pulse Vault. If Orca launches OMP, that process uses Orca's environment and native OMP state or authentication unless an operator configures another boundary explicitly. Pulse signing, linking, bootstrap, and relay keys are never OMP provider credentials.
