...
A separate ProviderCommandReactor empty-history compaction test fails at its activity assertion both with and without the new filtered read. Its worker drain does not establish that the subscribed domain stream has enqueued the dispatched intent. This batch does not alter that synchronization seam or weaken the assertion. No live providers or runtime clients were launched.
