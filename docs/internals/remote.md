...
Pulse Code has one runtime boundary: a client talks to a Pulse Code server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one Pulse Code server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Pulse Code server        │
...
One running Pulse Code server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed Pulse Connect relay tunnels.                                     |
...
Tailscale is the first provider, and Pulse Code manages more than discovery. When `tailscaleServeEnabled` is
...
Access answers one question: how does the client speak WebSocket to a Pulse Code server? It does not answer
...
Managed Pulse Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
A Pulse Code-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote Pulse Code
...
Launch answers a different question: how does a Pulse Code server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs Pulse Code and the client connects directly or through a
  tunnel.
...
- **Client-managed local publish.** A local server is published through the relay with
  `pulse connect link` (`t3 connect link` remains an alias), exposing a desktop-hosted environment to mobile without router or firewall
  changes.
