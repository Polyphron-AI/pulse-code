...
The screenshot harness runs the real mobile application against three disposable local Pulse Code
...
the software keyboard, and supplies deterministic Pulse Connect discovery rows to the real
...
1. Creates three temporary Pulse Code base directories and starts a local server for each on an available
   port.
2. Creates Pulse Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Pulse Code review diff.
...
The Pending rows use the production offline outbox and point at the real Pulse Code and React fixture
