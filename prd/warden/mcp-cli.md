...
Proposed 2026-09-14: the Go remote MCP server at `/mcp` is the primary agent-facing host for this catalogue, gated by `warden:read` and `warden:use` OAuth scopes that are never default. Callers without a trusted attempt assertion bind a use to their OAuth session and request digest, and results report that binding. Setup is a copied client snippet or a token-free connection code that Pulse desktop Settings can paste to register the server and start OAuth.
